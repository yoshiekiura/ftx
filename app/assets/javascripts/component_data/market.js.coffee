@MarketData = flight.component ->
  ...
  @after 'initialize', ->
    ...
    $('#stop-buy').click ->
      if $(@).prop('checked')
        $('#stop-buy-limit').slideDown 'fast'
      else
        $('#stop-buy-limit').slideUp 'fast'

    $('#stop-sell').click ->
      if $(@).prop('checked')
        $('#stop-sell-limit').slideDown 'fast'
      else
        $('#stop-sell-limit').slideUp 'fast'
    ...
    $('#bestSell').click ->
      ...
      if($("#order_ask_price").val()=="")
        ...
        if(currency.includes('XRP'))
          $('#order_ask_origin_volume').val(BigNumber(bestSell_).round(6, BigNumber.ROUND_CEIL).toF(6));
        else
          $('#order_ask_origin_volume').val(BigNumber(bestSell_).round(8, BigNumber.ROUND_CEIL).toF(8))
    ...
    $('#available_b').click ->
      ...
      if($('#volume_buy').html().includes('XRP'))
        if(BigNumber(divXrp).isNaN() || BigNumber(divXrp).isNegative())
         $('#order_bid_origin_volume').val(0);
        else
         $('#order_bid_origin_volume').val(BigNumber(divXrp));
        ...
        if(BigNumber(divCoin).isNaN() || BigNumber(divCoin).isNegative()  || divCoin == Infinity)
          $('#order_bid_origin_volume').val(BigNumber(0));
        else
          $('#order_bid_origin_volume').val(BigNumber(divCoin));
