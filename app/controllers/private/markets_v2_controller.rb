module Private
  class Marketsv2Controller < BaseController
    #include NewRelic::Agent::MethodTracer
    before_action :auth_member!
    before_action :visible_market?
    before_action :check_financial, only: [:show]
    after_action :set_default_market

    before_action :auth_activated!
    before_action :auth_verified!

    layout false

    def show
      @bid = params[:bid]
      @ask = params[:ask]

      @market        = current_market
      @markets       = Market.all_no_filter.sort
      @market_groups = @markets.map(&:quote_unit).uniq

      @bids   = @market.bids
      @asks   = @market.asks
      @trades = @market.trades

      # default to limit order
      @order_bid = OrderBid.new ord_type: 'limit'
      @order_ask = OrderAsk.new ord_type: 'limit'

      set_member_data if current_user
      gon.jbuilder

    end

    private

    def visible_market?
      redirect_to market_path(Market.first) if not current_market.visible?
    end

    def check_financial
      if is_financial?
        redirect_to root_path
      end

    end

    def set_default_market
      cookies[:market_id] = @market.id
    end

    def set_member_data
      @member = current_user
      @orders_wait = @member.orders.with_state(:wait)
      @trades_done = Trade.for_member(@market.id, current_user, limit: 100, order: 'id desc')
    end
    # add_method_tracer :show
    # add_method_tracer :visible_market?
    # add_method_tracer :set_default_market
    # add_method_tracer :set_default_market
    # add_method_tracer :set_default_market
  end
end
