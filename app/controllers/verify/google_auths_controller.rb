module Verify
  class GoogleAuthsController < ApplicationController
    ...
    def google_auth_activated?
      redirect_to settings_path, notice: t('verify.google_auths.show.notice.already_activated') if @google_auth.activated?
    end

    def google_auth_inactivated?
      redirect_to settings_path, notice: t('verify.google_auths.show.notice.not_activated_yet') if not @google_auth.activated?
    end
