...
#Cointrade alteracao para implementacao de bitvalor sellside e buyside
...
#Cointrade fim da alteracao
    ...
    def for_member(currency, member, options={})
      trades = filter(currency, options[:time_to], options[:from], options[:to], options[:limit], options[:order], options[:originator]).where("ask_member_id = ? or bid_member_id = ?", member.id, member.id)
