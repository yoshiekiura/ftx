class Deposit < ActiveRecord::Base
  #require 'newrelic_rpm'
  # include NewRelic::Agent::Instrumentation::ControllerInstrumentation
  STATES = [:submitting, :cancelled, :submitted, :rejected, :accepted, :checked, :warning]

  extend Enumerize

  include AASM
  include AASM::Locking
  include Currencible

  has_paper_trail on: [:update, :destroy]

  enumerize :aasm_state, in: STATES, scope: true

  alias_attribute :sn, :id

  delegate :name, to: :member, prefix: true
  delegate :id, to: :channel, prefix: true
  delegate :coin?, :fiat?, to: :currency_obj

  belongs_to :member
  belongs_to :account

  validates_presence_of \
    :amount, :account, \
    :member, :currency
  validates_numericality_of :amount, greater_than: 0

  scope :recent, -> { order('id DESC')}

  after_update :sync_update
  after_create :sync_create
  after_destroy :sync_destroy

  aasm :whiny_transitions => false do
    state :submitting, initial: true, before_enter: :set_fee
    state :cancelled
    state :submitted
    state :rejected
    state :accepted, after_commit: [:do, :send_mail]
    state :checked
    state :warning
    state :waiting

    event :submit do
      transitions from: :submitting, to: :submitted
    end

    event :cancel do
      transitions from: :submitting, to: :cancelled
    end

    event :reject do
      transitions from: :submitted, to: :rejected
    end

    event :accept do
      transitions from: :submitted, to: :accepted
    end

    event :check do
      transitions from: :accepted, to: :checked
    end

    event :warn do
      transitions from: :accepted, to: :warning
    end
  end

  def txid_desc
    txid
  end

  class << self
    # include NewRelic::Agent::Instrumentation::ControllerInstrumentation
    def channel
      DepositChannel.find_by_key(name.demodulize.underscore)
    end

    def resource_name
      name.demodulize.underscore.pluralize
    end

    def params_name
      name.underscore.gsub('/', '_')
    end

    def new_path
      "new_#{params_name}_path"
    end
    # add_transaction_tracer :new_path,
    #                        :name => 'new_path',
    #                        :category => "Deposit/new_path"
    # add_transaction_tracer :params_name,
    #                        :name => 'params_name',
    #                        :category => "Deposit/params_name"
    # add_transaction_tracer :resource_name,
    #                        :name => 'resource_name',
    #                        :category => "Deposit/resource_name"
    # add_transaction_tracer :channel,
    #                        :name => 'channel',
    #                        :category => "Deposit/channel"
  end

  def channel
    self.class.channel
  end

  def update_confirmations(data)
    update_column(:confirmations, data)
  end

  def txid_text
    txid && txid.truncate(40)
  end

  private
  def do
    account.lock!.plus_funds amount, reason: Account::DEPOSIT, ref: self
  end

  def send_mail
    begin
      DepositMailer.accepted(self.id) if self.accepted?
    rescue
      puts "Fail send mail"
    end

  end

=begin
  def send_sms
    return true if not member.sms_two_factor.activated?

    sms_message = I18n.t('sms.deposit_done', email: member.email,
                                             currency: currency_text,
                                             time: I18n.l(Time.now),
                                             amount: amount,
                                             balance: account.balance)

    AMQPQueue.enqueue(:sms_notification, phone: member.phone_number, message: sms_message)
  end
=end

  def set_fee
    amount, fee = calc_fee
    self.amount = amount
    self.fee = fee
  end

  def calc_fee
    [amount, 0]
  end

  def sync_update
    ::Pusher["private-#{member.sn}"].trigger_async('deposits', { type: 'update', id: self.id, attributes: self.changes_attributes_as_json })
    Rails.cache.write "peatio:deposit:#{self.txid_by_user}:#{self.currency}:data", self
  end

  def sync_create
    ::Pusher["private-#{member.sn}"].trigger_async('deposits', { type: 'create', attributes: self.as_json })
  end

  def sync_destroy
    ::Pusher["private-#{member.sn}"].trigger_async('deposits', { type: 'destroy', id: self.id })
  end
  # add_transaction_tracer :sync_destroy,
  #                        :name => 'sync_destroy',
  #                        :category => "Deposit/sync_destroy"
  # add_transaction_tracer :sync_create,
  #                        :name => 'sync_create',
  #                        :category => "Deposit/sync_create"
  # add_transaction_tracer :sync_update,
  #                        :name => 'sync_update',
  #                        :category => "Deposit/sync_update"
  # add_transaction_tracer :calc_fee,
  #                        :name => 'calc_fee',
  #                        :category => "Deposit/calc_fee"
  # add_transaction_tracer :set_fee,
  #                        :name => 'set_fee',
  #                        :category => "Deposit/set_fee"
  #
  # add_transaction_tracer :send_mail,
  #                        :name => 'send_mail',
  #                        :category => "Deposit/send_mail"
  # add_transaction_tracer :txid_text,
  #                        :name => 'txid_text',
  #                        :category => "Deposit/txid_text"
  # add_transaction_tracer :update_confirmations,
  #                        :name => 'update_confirmations',
  #                        :category => "Deposit/update_confirmations"
  # add_transaction_tracer :channel,
  #                        :name => 'channel',
  #                        :category => "Deposit/channel"
  # add_transaction_tracer :txid_desc,
  #                        :name => 'txid_desc',
  #                        :category => "Deposit/txid_desc"
end
