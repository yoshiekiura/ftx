class Member < ActiveRecord::Base
  ...
  class << self
    ...
    def financials
      Figaro.env.financial.split(',')
    end

    def supporters
      Figaro.env.support.split(',')
    end
    ...
    def search(field: nil, term: nil)
      result = case field
               ...
               when 'client_uuid'
                 joins(:identity).where('identities.client_uuid LIKE ?', "%#{term}%")
               when 'wallet_address'
  ...
  def financial?
    @is_financial ||= self.class.financials.include?(self.email)
  end

  def supporter?
    @is_supporter ||= self.class.supporters.include?(self.email)
  end
  ...
  def send_password_changed_notification
    MemberMailer.reset_password_done(self.id)
  ...
  def generate_sn
    self.sn and return
    begin
      self.sn = "CXC#{ROTP::Base32.random_base32(8).upcase}TRA"
