...
module Matching
  class LimitOrder
    ...
    def initialize(attrs)
      #puts "LIMIT ORDER INITIZALIZATION: ATTRS => #{attrs}"
      
      @id         = attrs[:id]
      @timestamp  = attrs[:timestamp]
      @type       = attrs[:type].to_sym if !attrs[:type].nil?
      @volume     = attrs[:volume].to_d if !attrs[:volume].nil?
      @price      = attrs[:price].to_d if !attrs[:price].nil?
      @market     = Market.find attrs[:market] if !attrs[:market].nil?
