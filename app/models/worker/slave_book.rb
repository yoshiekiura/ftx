  ...
  class SlaveBook
    ...
    def get_depth(market, side)
      ...
      @managers[market.id].send("#{side}_orders").limit_orders.each do |price, orders|
        ...
        depth[price] += [orders.map(&:volume).count, orders.map(&:volume).sum]
      end
      depth = depth.to_a
     ...
     add_transaction_tracer :cache_book, :category => :task
     add_transaction_tracer :order, :category => :task
     add_transaction_tracer :book, :category => :task
     add_transaction_tracer :manager, :category => :task
     add_transaction_tracer :initialize_orderbook_manager, :category => :task
     add_transaction_tracer :get_depth, :category => :task
