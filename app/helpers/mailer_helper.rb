module MailerHelper
  ...
  def assets_value_change_total(changes)
    total = changes.sum do |(currency, amount, value)|
      currency.code == 'brl' ? 0 : (value[0] || 0)
    end
    pretty_change pretty_currency(total, 'brl'), total
  end
  ...
  def email_image_tag(img)
    attachments.inline["#{img}"] = File.read(Rails.root.join("app/assets/images/#{img}"))
    image_tag(attachments["#{img}"].url)
  end
