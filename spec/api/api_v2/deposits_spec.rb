...
describe APIv2::Deposits do
  ...
  describe "GET /api/v2/deposits" do
    ...
    before do
      ...
      create(:deposit, member: member, currency: 'brl')
      create(:deposit, member: member, currency: 'brl', txid: 1, amount: 520)
      create(:deposit, member: member, currency: 'btc', created_at: 2.day.ago,  txid: 'test', amount: 111)
      create(:deposit, member: other_member, currency: 'brl', txid: 10)
    end
    ...
    it "should filter deposits by state" do
      ...
      token = create(:api_token, member: member)
    ...
    it "deposits currency brl" do
      signed_get '/api/v2/deposits', params: {currency: 'brl'}, token: token
      result = JSON.parse(response.body)
      result.should have(2).deposits
      result.all? {|d| d['currency'] == 'brl' }.should be_true
    end
