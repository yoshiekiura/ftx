...
describe 'Sign up', js: true do
  ...
  def fill_in_sign_up_form
    ...
    within('form#new_identity') do
      ...
      click_on "Termos e condições de uso"
    end

    within('.modal') do
      page.execute_script "$('.modal-content').scrollTop($('.modal-content')[0].scrollHeight)"
      check = first("input[type='checkbox']")
      check.set(true)
    end
    click_on I18n.t('header.signup').upcase
  end
  ...
  it 'allows a user to sign up if limit not reached' do
    fill_in_sign_up_form
    clear_cookie
    signin identity
    check_signin
    expect(page).not_to have_content(I18n.t('.verification.title'))
    visit signout_path
    ...
    fill_in_sign_up_form
    expect(page).not_to have_content(I18n.t('activerecord.errors.messages.limit_excedent'))
  end
  ...
  it 'allows a user to sign up and activate the account without kyc' do
    fill_in_sign_up_form
    clear_cookie
