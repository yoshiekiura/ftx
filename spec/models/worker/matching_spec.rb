...
describe Worker::Matching do
  ...
  context "engines" do
    it "should get all engines" do
      subject.engines.keys.should == Market.to_hash.keys.as_json
    end

    it "should started all engines" do
      subject.engines.values.map(&:mode).count.should == Market.all.count
    end
  end
  ...
  context "partial match" do
    ...
    it "should started engine" do
      subject.engines['btcbrl'].mode.should == :run
    end
    ...
    it "should match part of existing order" do
      ...
      AMQPQueue.expects(:enqueue)
        .with(:slave_book, {action: 'update', order: {id: existing.id, timestamp: existing.at, type: :ask, volume: '2.0'.to_d, price: existing.price, market: 'btcbrl', ord_type: 'limit'}}, anything)
      AMQPQueue.expects(:enqueue)
  ...
  context "complex partial match" do
    ...
    let!(:orderbook) { Matching::OrderBookManager.new('btcbrl', broadcast: false) }
  ...
  context "dryrun" do
    ...
    context "very old orders matched" do
      ...
      it "should not start engine" do
        subject.engines['btcbrl'].mode.should == :dryrun
        subject.engines['btcbrl'].queue.should have(1).trade
      end
    end
